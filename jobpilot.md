...
> 本文面向项目维护者、学习者和面试复盘场景，基于 2026-09-15 的代码状态整理。当前评分引擎版本为 `evidence-v3`。
...
`evidence-v3` 中，通用职责判断必须包含岗位要求的具体词语。只出现“产品、项目、数据”等宽泛词时，最多只能得到部分匹配；动作型 JD 句子会归入职责，不会再被误识别成岗位名称。
...
LLM 结果不能覆盖规则已经明确找到的要求。`evidence-v3` 会合并两者，并保留 JD 中显式出现的英文技术名词，避免 LLM 只返回一两个要求后仍将分数归一化成 100。
...
- 证据句必须存在具体岗位词锚点；
- 仅命中宽泛词最多部分匹配；
- 通用 JD 中带明确强制措辞的技能，以及明确的经验和学历要求启用硬门槛；
- LLM 解析结果与规则结果合并。
- `协助、完成、开发、联调、集成、编写、调试、调优、构建` 等动作句归入职责；
- 通用 JD 中的首字母大写技术词和缩写进入评分项；
- 只有带“必须、至少、精通、mandatory”等明确措辞的技能才触发严重硬门槛封顶。
